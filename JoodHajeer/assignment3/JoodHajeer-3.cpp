...
private:
	...
	//void prnt(DictionaryNode * );
	DictionaryNode * root;
};
/*
void Dictionary::prnt() {
	prnt(root);
}
void Dictionary::prnt(DictionaryNode * node) {
	if (node == nullptr)
		return;
	for (int i = 0; i < 26; i++) {
		if (node->next[i]!=nullptr && node->next[i]->isLeaf)
			cout <<"leaf: "<<(char) ('A' + i)<<endl;
		prnt(node->next[i]);
	}
}
*/
...
void DFS(int curRow, int curCol ,string curWord, vector<vector<bool> > & visited, vector<vector<char> >& charGrid, Dictionary& dictionary_, set<string> & words) {
	...
	if (dictionary_.isPrefix(curWord)) {
		for (int k = 0; k < sizeof(dRow); k++) {
			...
			if (newRow < 0 || newRow >= charGrid.size() || newCol < 0 || newCol >= charGrid[0].size())
				continue;
...
void findWords(Dictionary& dictionary_, vector<vector<char> >& charGrid, set<string> & words) {
	...
	visited.resize(charGrid.size(), vector<bool>(charGrid[0].size(), false));
	string curWord = "";
...
//////////////////////////////////////////////////
//testing begins here
bool equalVectors(vector<string> actual_vec, vector<string> expected_vec) {
...
void testFindWords_dictionaryWithDuplicates(Dictionary & dictionary_, vector<vector<char> > & charGrid, set<string> & words) {;
	...
	if(!equalVectors(expected_vec, actual_vec))
