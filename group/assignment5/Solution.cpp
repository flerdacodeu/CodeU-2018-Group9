...
class AlphabetFinder
{
	...
	//the actual graph, map value: set of chars that are lexicographically AFTER the key char
	...
	void buildGraph()
	{
		...
		//map[mismatching letter from the first word] = mismatching letter from the second word,
		//because the letter from the second word should come AFTER the one from the first word
		int firstLoopIterations = words.size() - 1; //if we have 1 2 3 words, the pairs are: 1 2, 2 3, so 3 words -> 2 pairs
		for (int i = 0; i < firstLoopIterations; i++)
		{
			string word1 = words[i];
			string word2 = words[i + 1];
			int secondLoopIterations = min(word1.length(), word2.length());//we'll stop if the end of any of the 2 words is reached
			for (int j = 0; j < secondLoopIterations; j++)
			{
				if (word1[j] != word2[j])
				{
					charsMap[word1[j]].emplace(word2[j]);
					break; //we shouldn't check the rest of the 2 words, so we should move on to another pair
				}
			}
		}
	}
	...
	void visit(char c)
	{
		...
		//all the chars that should come after it are added to the stack before it,
		//so if we have a<b & b<c,
		//when a is visited, it's not added to the stack, instead, it calls visit(b)
		//when b is visited, it's not added to the stack, instead, it calls visit(c)
		//when c is visited, nothing comes after it, so it's added to the stack, 
		//then we go back to b, that called visit(c), and b is added to the stack,
		//then we go back to a, that called visit(b), and a is added to the stack,
		//when we empty the stack, they come out in this order: a b c, which is the correct order
		unordered_set<char>::iterator it;
